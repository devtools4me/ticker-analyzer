package me.devtools4.ticker.listing.effect
...
trait IO[A] {
  ...
  def flatMap[Next](next: A => IO[Next]): IO[Next] =
    IO.async { cb =>
      unsafeRunAsync {
        ...
        case Success(v) => next(v).unsafeRunAsync(cb)
      }
    }
  ...
  def onError[Other](cleanup: Throwable => IO[Other]): IO[A] =
    handleErrorWith(ex => cleanup(ex).andThen(IO.fail(ex)))
  ...
  def retry(maxAttempt: Int): IO[A] = {
    if (maxAttempt < 1) IO.fail(new IllegalArgumentException())
  ...
  def parZip[Other](other: IO[Other])(ec: ExecutionContext): IO[(A, Other)] =
    IO.async { cb =>
      val p1: Promise[A] = Promise()
      val p2: Promise[Other] = Promise()
      ec.execute(() => unsafeRunAsync(p1.complete))
      ec.execute(() => other.unsafeRunAsync(p2.complete))
...
object IO {
  ...
  def async[A](onComplete: (Try[A] => Unit) => Unit): IO[A] = new IO[A] {
    override def unsafeRunAsync(cb: Try[A] => Unit): Unit =
      onComplete(cb)
  }
  ...
  def sequence[A](actions: List[IO[A]]): IO[List[A]] =
    actions.foldLeft(IO(List.empty[A]))((lhs, rhs) =>
      lhs.zip(rhs).map {
        case (l, e) => e +: l
      })
      .map(_.reverse)
